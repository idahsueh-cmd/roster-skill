# Cowork — Status

Give me a current snapshot of this collaboration session. Do not ask me anything — just read and summarize.

**Step 1 — Read `COWORK.md`** at the repo root. If `COWORK-detail.md` exists, read that too.

**Step 2 — Output a one-screen summary with these sections:**

**Who owns what**
A condensed version of the ownership table — not the raw table, just the key assignments in plain language.

**Needs agreement (unresolved)**
List any files/directories marked `ALL` in the Ownership table that have no logged resolution in `## Open Conflicts`.

**Open conflicts**
Any disputes currently logged in `## Open Conflicts` that are not yet resolved.

**Next step for each person**
Based on what you read, what should each collaborator be doing or watching out for right now?

Keep the entire output under one screen. Be direct and specific.
